...
class CaseProgressStatusAPITest(APITestCase):
    def setUp(self):
        ...
        assign_role(self.other_student, 'student')
        self.professor = User.objects.create_user(username='professor_progress', password='pass')
        assign_role(self.professor, 'professor')
    ...
    def test_assigned_professor_can_list_progress_statuses(self):
    ...
    def test_assigned_professor_can_add_progress_status(self):
        self.client.force_authenticate(self.professor)
        response = self.client.post(self.url, {'label': 'Revision legal completada'}, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
